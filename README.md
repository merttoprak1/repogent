...
When `--model` is omitted, Repogent leaves model choice to Codex and records `default` in its evidence. Pass `--model NAME` to request an explicit Codex model; Repogent passes that value once to each `codex exec` invocation. The provider performs separate readiness checks (executable, structured-exec capability, and login state), writes a `provider-readiness` artifact, and records a `provider-call` artifact and usage for every model role. Each role generation is one structured `codex exec` call—there is no provider-side retry or hidden second proposal call.
...
- provider readiness, per-call evidence and usage, and the independent QA result.
