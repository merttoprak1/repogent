...
class _PythonSymbolVisitor(ast.NodeVisitor):
    ...
    def _source(self) -> str:
        return ".".join((self.module_name, *self._qualified_names))
    ...
    def visit_Import(self, node: ast.Import) -> None:
        for imported in node.names:
            self.edges.append(SymbolEdge(source=self._source, target=imported.name, kind="imports"))

    def visit_ImportFrom(self, node: ast.ImportFrom) -> None:
        module = "." * node.level + (node.module or "")
        for imported in node.names:
            target = f"{module}.{imported.name}" if module else imported.name
            self.edges.append(SymbolEdge(source=self._source, target=target, kind="imports"))
    ...
    def _visit_symbol(
        ...
        parent_id = self._symbol_ids[-1] if self._symbol_ids else None
