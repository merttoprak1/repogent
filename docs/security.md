...
- Patch application snapshots every touched path and attempts restoration after ordinary failures, `KeyboardInterrupt`, or `SystemExit`. If restoration cannot be proved, checkout state becomes `recovery_unknown` and every touched path is surfaced for manual inspection.
